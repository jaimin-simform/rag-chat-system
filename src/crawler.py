...
async def get_title_and_summary(chunk: str, url: str) -> Dict[str, str]:
    """Extract title and summary using GPT-3.5"""
    system_prompt = """You are an AI that extracts titles and summaries from web content chunks.
    Return a JSON object with 'title' and 'summary' keys.
    For the title: If this seems like the start of a document, extract its title. If it's a middle chunk, derive a descriptive title.
    For the summary: Take al lthe text available on that web page in this chunk.
    Keep both title and summary"""
...
async def crawl_parallel(urls: List[str], max_concurrent: int = 5):
    ...
    print(f"DEBUG: Found {len(urls)} URLs to crawl")  # Debugging
    ...
    try:
        ...
        async def process_url(url: str):
            ...
            async with semaphore:
                ...
                print(f"DEBUG: Result received for {url}")
                if result.success:
                    processed_urls += 1
                    print(
